...
describe('verify', async () => {
  let program: Program<Chain | Idl>, programId: PublicKey;
  ...
  it('creates block, using blockId to generate pda', async () => {
    ...
    expect(decodeBlockRoot((await program.account.block.fetch(blockPda)).root)).to.equal(
      '0x94ee327959d93a3cec35639bac830d5dc37c0f20ecd0e9cfa47b59d6829de605',
    );
  ...
  it('fails to create another block with the same information as the previous one', async () => {
    try {
      ...
      expect(
        err.logs.find((log) => log.includes('Allocate: account Address') && log.includes('already in use')) !==
          undefined,
      );
    }
  });
